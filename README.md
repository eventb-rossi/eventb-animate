...
```yaml
- uses: eventb-rossi/eventb-animate@v6.5
  with:
...
| `version` | Release version tag (e.g., `v6.5`); pin it for a reproducible run | No | `latest` |
...
- id: setup-eventb-animate
  uses: eventb-rossi/eventb-animate/setup@v6.5
  with:
    version: 'v6.5'
    java-version: ''
...
| Input | Description | Required | Default |
|-------|-------------|----------|---------|
| `version` | Release version tag (e.g., `v6.5`); pin it for a reproducible install | No | `latest` |
...
```yaml
include:
  - remote: 'https://raw.githubusercontent.com/eventb-rossi/eventb-animate/v6.5/.gitlab-ci-template.yml'
...
| `EVENTB_ANIMATE_VERSION` | Release version tag (e.g., `v6.5`) | `latest` |
